...
from utils.common_functions import read__yaml, load_data
...
class DataProcessor:
    ...
    def __init__(self, train_path, test_path, processed_dir,config_path):
        ...
        self.config = read__yaml(config_path)
        # self.categorical_columns = self.config["data_processing"]["categorical_columns"]
        # self.numerical_columns = self.config["data_processing"]["numerical_columns"]
    ...
    def preprocess_data(self,df):
        try:
            ...
            logger.info("Droppping the columns")
            ...
            for col, mappings in mappings.items():
                logger.info(f"Mappings for {col}: {mappings}")
            ...
            for column in skewness[skewness> skew_threshold].index:
                ...
                logger.info(f"Applied log transformation to {column} due to skewness")
    ...
    def balance_data(self, df):
        try:
            ...
            logger.error("Error while handling imbalanced data")
...
if __name__=="__main__":
    processor = DataProcessor(TRAIN_FILE_PATH,TEST_FILE_PATH,PROCESSED_DIR,CONFIG_PATH) #comes from paths_config
    processor.process()       
